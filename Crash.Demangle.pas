...
function CppSymbolToPascal(const ASymbol: String): String;
...
function CppSymbolToPascal(const ASymbol: String): String;
begin
  Result := TSymbolConverter.CppToPascal(ASymbol);
end;

{ TSymbolConverter }

class function TSymbolConverter.ConvertCtorDtor(const AStr: String): String;
...
class function TSymbolConverter.ConvertOperatorName(
...
class function TSymbolConverter.ConvertSet(const AStr: String): String;
{ Set<XYZ, ...> -> set of XYZ }
var
  I: Integer;
begin
...
class function TSymbolConverter.ConvertSpecialFunctionName(
...
class function TSymbolConverter.ConvertStaticArray(
  const AStr: String): String;
...
class function TSymbolConverter.CppToPascal(const ASymbol: String): String;
...
class constructor TSymbolConverter.Create;
begin
...
class destructor TSymbolConverter.Destroy;
begin
  FTypeMap.Free;
  FOperatorMap.Free;
end;

class function TSymbolConverter.ParseFunctionHeading(var AStr: PChar): String;
...
class function TSymbolConverter.ParseGenericArg(var AStr: PChar): String;
...
class function TSymbolConverter.ParseGenericArgs(var AStr: PChar): String;
...
class function TSymbolConverter.ParseName(var AStr: PChar): String;
...
class function TSymbolConverter.ParseParam(var AStr: PChar): String;
{ Param -> QualifiedName+
...
class function TSymbolConverter.ParseParams(var AStr: PChar): String;
{ Params -> Param (',' Param)*
...
class function TSymbolConverter.ParseQualifiedName(var AStr: PChar): String;
var
  Dummy: Boolean;
begin
...
class function TSymbolConverter.ParseQualifiedName(var AStr: PChar;
...
class function TSymbolConverter.ParseRemainder(var AStr: PChar): String;
...
class procedure TSymbolConverter.SkipWhitespace(var AStr: PChar);
begin
  while (AStr^ <> #0) and (AStr^ <= ' ') do
    Inc(AStr);
end;
